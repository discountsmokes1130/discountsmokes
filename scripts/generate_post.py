...
def ensure_structure():
    """Make sure posts/ and index.json exist; verify topics.json is present."""
...
def read_topics():
    """Load topics from posts/topics.json."""
...
def get_next_index(total: int) -> int:
    """Read the next topic index from state (wraps to 0 if out of range)."""
...
def bump_index(idx: int, total: int):
    """Persist the next index so the next run advances the rotation."""
    next_idx = (idx + 1) % total
    STATE_FILE.write_text(json.dumps({"next_index": next_idx}, indent=2), encoding="utf-8")


def gen_with_openai(prompt: str) -> str:
    """Call OpenAI to generate content (or return placeholder in DRY_RUN)."""
    if DRY_RUN or not OPENAI_API_KEY:
        # Fallback content so pipeline still produces a post
...
def main():
    ...
    # Pick next topic by persistent index (not date-based → no “stuck” repeats)
    idx = get_next_index(len(topics))
    topic = topics[idx]
    category = topic.get("category", "General")
    idea = topic.get("idea") or topic.get("title") or "Store update"

    # Title + content prompts (FIXED multiline f-strings)
